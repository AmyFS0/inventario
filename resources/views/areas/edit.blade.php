<x-app-layout>
    <div class="max-w-3xl mx-auto space-y-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Editar área</h1>
                <p class="text-sm text-gray-500 mt-1">{{ $area->nombre }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('areas.update', $area) }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select name="encargado_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach ($encargados as $encargado)
                                <option value="{{ $encargado->id }}" @selected(old('encargado_id', $area->encargado_id) == $encargado->id)>{{ $encargado->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="descripcion" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descripcion', $area->descripcion) }}</textarea>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Actualizar área</button>
                </div>
            </form>
        </div>
    </div>
