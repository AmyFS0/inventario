<x-app-layout>
    <div class="max-w-3xl mx-auto space-y-4">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Registrar ajuste</h1>
                <p class="text-sm text-gray-500 mt-1">Corrige el stock: use cantidad positiva para incrementar y negativa para reducir.</p>
            </div>
            <a href="{{ route('movimientos.index') }}" class="text-sm text-gray-600 hover:text-gray-900">← Volver</a>
        </div>

        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form method="POST" action="{{ route('movimientos.ajuste.store') }}" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Cantidad * <span class="text-gray-400">(puede ser negativa)</span></label>
                        <input type="number" step="0.01" name="cantidad" id="cantidad" value="{{ old('cantidad') }}" required placeholder="Ej. 5 o -3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('cantidad') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700">Motivo *</label>
                        <input type="text" name="motivo" value="{{ old('motivo') }}" required placeholder="Ej. Conteo físico, merma detectada"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-md bg-amber-600 text-white text-sm font-medium hover:bg-amber-700">Registrar ajuste</button>
                </div>
            </form>
        </div>
    </div>
        ...
        (async () => {
            ...
            async function consultarStock() {
                ...
                try {
                    ...
                    if (cantidad !== 0) {
                        const resultado = actual + cantidad;
                        if (resultado < 0) {
                            msg += ' ⚠️ <span class="text-red-600">El ajuste dejaría stock negativo.</span>';
                            submitBtn.disabled = true;
                        } else {
                            msg += ' Resultado: <strong>' + resultado.toLocaleString('es-HN') + ' ' + unidad + '</strong>.';
