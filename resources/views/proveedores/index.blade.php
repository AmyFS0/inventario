    ...
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Proveedores que abastecen el inventario.</p>
            </div>
            @can('crear proveedores')
            <a href="{{ route('proveedores.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">
        ...
        <form method="GET" action="{{ route('proveedores.index') }}" class="flex gap-2">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Proveedor</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($proveedores as $proveedor)
                            <tr class="hover:bg-gray-50">
                                <td class="px-5 py-3 font-medium text-gray-900">{{ $proveedor->nombre }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ $proveedor->contacto }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ $proveedor->telefono }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ $proveedor->correo }}</td>
                                <td class="px-5 py-3 text-center text-gray-600">{{ $proveedor->items_count ?? $proveedor->items->count() }}</td>
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        @can('editar proveedores')
                                        <a href="{{ route('proveedores.edit', $proveedor) }}" class="text-amber-600 hover:text-amber-800 font-medium text-xs">Editar</a>
                                        @endcan
                                        @can('eliminar proveedores')
                                        <form method="POST" action="{{ route('proveedores.destroy', $proveedor) }}" onsubmit="return confirm('¿Seguro que deseas eliminar este proveedor?');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-10 text-center text-gray-500">No hay proveedores registrados.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
