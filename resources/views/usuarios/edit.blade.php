        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form method="POST" action="{{ route('usuarios.update', $usuario) }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="empresa_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach ($empresas as $empresa)
                                <option value="{{ $empresa->id }}" @selected(old('empresa_id', $usuario->empresa_id) == $empresa->id)>{{ $empresa->nombre }}</option>
                            @endforeach
                        </select>
                            ...
                            @foreach ($roles as $rol)
                                <label class="flex items-center gap-3 rounded-md border border-gray-200 px-4 py-3 cursor-pointer hover:bg-gray-50">
                                    <input type="checkbox" name="roles[]" value="{{ $rol->id }}"
                                        @checked(in_array($rol->id, $usuario->roles->pluck('id')->all()) || in_array($rol->id, (array) old('roles', [])))
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Actualizar usuario</button>
                </div>
            </form>
        </div>
    </div>
